...
=== 传输线的工作状态
传输线的工作状态由负载阻抗 $Z_L$ 和特性阻抗 $Z_0$ 决定

+ 行波状态

  此时 $Z_L = Z_0$，只有入射波，无反射波，电压、电流同相位

+ 驻波状态

  此时 $Z_L = cases(0, oo, plus.minus jj X_L)$，产生全反射，入射波与反射波叠加形成驻波
...
时刻 $t$ 空间任意一点 $bold(r)$ 处的位函数并不取决于该时刻的电流和电荷分布，而是取决于比 $t$ 较早的时刻的电流或电荷分布。滞后的时间正好为电磁波以速度 $1\/sqrt(mu epsilon)$ 从源点 $bold(r)'$ 传到场点 $bold(r)$ 所需的时间。所以称为*滞后位*

== 电偶极子的辐射

几何长度远小于波长的线元上载有等幅同相的电流，称为电偶极子。

电偶极子产生的电磁场，磁场只有 $bold(H)_(phi.alt)$ 分量，电场只有 $bold(E)_r  ," " bold(E)_theta $ 分量，其他分量都为零。


- 近区场的电磁波（$r << lambda$）
  + 电场表达式与静电偶极子的电场表达式相同；磁场表达式与用毕奥一萨伐定律计算的恒定电流元产生的磁场表达式相同。因此称其为*似稳场*或*准静态场*。
  + 电场和磁场存在 $pi/2$ 的相位差，能量在电场和磁场以及场与源之间交换，没有辐射（$bold(S)_("av") = bold(0)$），所以近区场也称感应场。

- 远区场（辐射场）的电磁波（$r >> lambda$）
  + 远区场是横电磁波，电场、磁场和传播方向相互垂直
  + 远区电场和磁场的相位相同
  + 电场振幅与磁场振幅之比等于媒质的本征阻抗
  + 远区场是非均匀球面波，电场、磁场的振幅与 $1/r$ 成正比
  + 在轴向上（$theta = 0 degree$），场强为零，在垂直于电偶极子轴线的方向上（$theta = 90 degree$），场强最大。

- 辐射功率
  $
    P_r = 40 pi^2 I^2(l / lambda)^2
  $

- 辐射电阻
  $
    R_r = (2 P_r) / I^2 = 80 pi^2 (l / lambda)^2
  $